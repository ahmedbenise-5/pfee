                        ...
                        <div class="card-body py-3">
                            <form action="{{ route('Presence.store') }}" method="POST">
                                        ...
                                        @if (isset($Etudiants))
                                            @foreach ($Etudiants as $Etudiant)
                                                <tr>
                                                    ...
                                                    <input type="hidden" name="id_niveauxdetudes"
                                                        value="{{ $Etudiant->id_niveauxdetudes }}">
                                                    <input type="hidden" name="id_classes"
                                                        value="{{ $Etudiant->id_classes }}">
                                                    <input type="hidden" name="id_sections"
                                                        value="{{ $Etudiant->id_sections }}">


                                                    <td>
                                                        @if (isset(
                                                                $Etudiant->Presence()->where('presences_date', date('Y-m-d'))->first()->id_etudiant))
                                                            <div class="d-flex justify-content flex-shrink-0">
                                                                <div class="form-check form-check-inline">
                                                                    <input class="form-check-input" type="radio"
                                                                        name="presnces[{{ $Etudiant->id }}]"
                                                                        value="present" disabled
                                                                        {{$Etudiant->Presence()->first()->presences_status == 1   ? 'checked' : '' }}>

                                                                    <span
                                                                        class="badge badge-light-success p-0">present</span>
                                                                </div>
                                                                <div class="form-check form-check-inline">
                                                                    <input class="form-check-input" type="radio"
                                                                        name="presnces[{{ $Etudiant->id }}]"
                                                                        value="absent" disabled
                                                                        {{$Etudiant->Presence()->first()->presences_status == 0  ? 'checked' : '' }}>
                                ...
                                <div class="card-footer d-flex justify-content-end py-6 px-9">
                                    <button class="btn btn-primary px-6" type="submit">Sauvegarder</button>
                                </div>
                            </form>
                        </div>
                    </div>
